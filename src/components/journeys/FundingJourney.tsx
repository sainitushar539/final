...
const FundingJourney = ({ lead, onBack }: JourneyProps) => {
  ...
  const scoreCaption =
    score >= 80 ? "Excellent - You're highly fundable!" :
    score >= 60 ? 'Good - Strong foundation' :
    score >= 40 ? "Building - Let's close the gaps" :
    ...
    <div className="flex flex-wrap gap-2">
      {options.map(o => (
        <button
          ...
          className={`cursor-pointer rounded-xl border bg-transparent px-4 py-2.5 text-sm font-medium transition-all ${
            selected === o.value ? 'border-[#2563eb]/50 bg-[#2563eb]/[0.08] text-white' : 'border-white/[0.1] text-white/50 hover:border-white/[0.2]'
          }`}
        >
          {selected === o.value && <Check className="mr-1.5 inline h-3 w-3" />}
          {o.label}
        </button>
      ))}
    </div>
  ...
  return (
    <JourneyShell onBack={onBack}>
      {phase === 'questions' && (
        <>
          <div className="mb-2">
            <span className="mb-3 inline-block rounded-full bg-[#2563eb]/[0.08] px-3 py-1.5 text-xs font-semibold text-[#2563eb]">Your Path to Funding</span>
            <h2 className="mb-1 text-2xl font-extrabold text-white">3 quick questions.</h2>
            <p className="mb-6 text-sm text-white/50">We'll generate your funding readiness score.</p>
          </div>
          <div className="space-y-6">
            <div>
              <label className="mb-3 block text-xs font-semibold text-white/70">What's your estimated credit score?</label>
              <Pills options={creditOpts} selected={credit} onSelect={setCredit} />
            </div>
            <div>
              <label className="mb-3 block text-xs font-semibold text-white/70">Annual revenue?</label>
              <Pills options={revenueOpts} selected={revenue} onSelect={setRevenue} />
            </div>
            <div>
              <label className="mb-3 block text-xs font-semibold text-white/70">How long have you been in business?</label>
          ...
          <button
            ...
            className="mt-8 w-full cursor-pointer rounded-xl border-none bg-gradient-to-r from-[#2563eb] to-[#1d4ed8] py-3.5 text-sm font-semibold text-white transition-all hover:shadow-lg disabled:cursor-not-allowed disabled:opacity-40"
          >
            See My Score
          </button>
        </>
      ...
      {phase === 'results' && (
        <>
          <div className="mb-6 text-center">
            <span className="mb-3 inline-block rounded-full bg-[#2563eb]/[0.08] px-3 py-1.5 text-xs font-semibold text-[#2563eb]">Your Funding Score</span>
            <div className="mb-1 text-6xl font-extrabold text-white">{score}</div>
            <p className="text-sm text-white/50">{scoreCaption}</p>
          </div>
          <AdviceCard
            advice={advice}
            loading={loading}
            error={error}
            onRetry={() => fetchAdvice('funding', { name: lead.name, credit, revenue, time, score })}
            loadingText="Generating your credit snapshot..."
            scoreValue={score}
            scoreLabel="Your Funding Score"
